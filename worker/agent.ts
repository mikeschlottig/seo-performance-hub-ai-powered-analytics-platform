...
export class ChatAgent extends Agent<Env, ChatState> {
  ...
  initialState: ChatState = {
    ...
    systemPrompt: 'You are a helpful AI SEO assistant.'
  };
  ...
  async onRequest(request: Request): Promise<Response> {
    try {
      ...
      if (method === 'POST' && url.pathname === '/system-prompt') return this.handleSystemPromptUpdate(await request.json());
  ...
  private async handleChatMessage(body: { message: string; model?: string; stream?: boolean }): Promise<Response> {
    ...
    try {
      ...
      const response = await this.chatHandler.processMessage(message, this.state.messages, undefined, this.state.systemPrompt);
      const assistantMessage = createMessage('assistant', response.content, response.toolCalls);
  ...
  private handleSystemPromptUpdate(body: { prompt: string }): Response {
    this.setState({ ...this.state, systemPrompt: body.prompt });
    return Response.json({ success: true, data: this.state });
  }
